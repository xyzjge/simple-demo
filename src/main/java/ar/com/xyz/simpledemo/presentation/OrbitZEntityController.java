...
public class OrbitZEntityController extends EntityController {
	...
	public void update(float fts) {
		...
		if (seconds > 10) {
			getEntity().increaseRotation(0, 0, fts * -100);
		}
		...
		getEntity().setPosition(
			...
			initialPosition.y + ((float)Math.cos(angle + Math.PI))*radio,
			radio
		);
